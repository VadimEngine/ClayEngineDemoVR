...
SandboxSceneGUI::SandboxSceneGUI(clay::ShaderProgram* pTextureShader, clay::Mesh* pPlaneMesh, SandboxScene* theScene)
...
SandboxSceneGUI::~SandboxSceneGUI() = default;

void SandboxSceneGUI::setInputHandler(clay::InputHandlerXR* pInputHandler) {
    mpInputHandler_ = pInputHandler;
}

void SandboxSceneGUI::render(clay::IGraphicsContext& gContext) {
    ...
    // return back to original frame buffer
    glBindFramebuffer(GL_FRAMEBUFFER, previousFramebuffer);
    renderPlane(gContext);
}

void SandboxSceneGUI::renderPlane(clay::IGraphicsContext& gContext) {
    // render plane
    mShader_->bind();
...
void SandboxSceneGUI::setPosition(const glm::vec3& position) {
    mPosition_ = position;
}

void SandboxSceneGUI::setRotation(const glm::vec3& rotation) {
    mRotation_ = rotation;
...
void SandboxSceneGUI::setScale(const glm::vec3& scale) {
    mScale_ = scale;
}

void SandboxSceneGUI::buildImGui(clay::IGraphicsContext& gContext) {
...
void SandboxSceneGUI::pointAt(glm::vec3 rayOrigin, glm::vec3 rayDir) {
